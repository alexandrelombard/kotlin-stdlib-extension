...
internal open class MutableBigInteger {
    ...
    private fun ones(n: Int) {
        if (n > value.size) value = IntArray(n)
        value.fill(-1)
        offset = 0
        intLen = n
    }
    ...
    fun toBigDecimal(sign: Int, scale: Int): BigDecimal {
        ...
        val v =
            (if (len == 2) mag[1].toLong() and BigInteger.LONG_MASK or (d.toLong() and BigInteger.LONG_MASK) shl 32 else d.toLong() and BigInteger.LONG_MASK).toLong()
    ...
    fun toCompactValue(sign: Int): Long {
        ...
        // make a BigInteger object for the resultant BigDecimal object.
        if (len > 2 || d < 0 && len == 2) return BigDecimal.INFLATED
        val v =
            (if (len == 2) mag[1].toLong() and BigInteger.LONG_MASK or (d.toLong() and BigInteger.LONG_MASK) shl 32 else d.toLong() and BigInteger.LONG_MASK).toLong()
    ...
    fun rightShift(n: Int) {
        ...
        intLen -= nInts
        if (nBits == 0) return
        val bitsInHighWord: Int = BigInteger.bitLengthForInt(value[offset])
    ...
    fun leftShift(n: Int) {
        ...
        if (value.size < newLen) {
            ...
            // Use space on right
            for (i in 0 until newLen - intLen) value[offset + intLen + i] = 0
        } else {
            // Must use space on left
            for (i in 0 until intLen) value[i] = value[offset + i]
            for (i in intLen until newLen) value[i] = 0
            offset = 0
        }
        intLen = newLen
        if (nBits == 0) return
    ...
    private fun mulsub(q: IntArray, a: IntArray, x: Int, len: Int, offset: Int): Int {
        ...
        for (j in len - 1 downTo 0) {
            ...
            carry = (product ushr 32) + if (difference and BigInteger.LONG_MASK > product.toInt().inv().toLong() and BigInteger.LONG_MASK) 1 else 0
        }
        return carry.toInt()
    }
    ...
    private fun getLower(n: Int): BigInteger {
        return if (isZero) {
            ...
            BigInteger(value.copyOfRange(offset + intLen - len, offset + intLen), sign)
        }
    }
    ...
    fun add(addend: MutableBigInteger) {
        ...
        while (x > 0 && y > 0) {
            ...
            sum = (value[x + offset].toLong() and BigInteger.LONG_MASK) +
                    (addend.value.get(y + addend.offset).toLong() and BigInteger.LONG_MASK) + carry
        ...
        while (y > 0) {
            y--
            sum = (addend.value.get(y + addend.offset).toLong() and BigInteger.LONG_MASK) + carry
            result[rstart--] = sum.toInt()
            carry = sum ushr 32
        }
    ...
    fun addShifted(addend: MutableBigInteger, n: Int) {
        ...
        while (x > 0 && y > 0) {
            x--
            y--
            val bval = if (y + addend.offset < addend.value.size) addend.value.get(y + addend.offset) else 0
            sum = (value[x + offset].toLong() and BigInteger.LONG_MASK) +
    ...
    fun addDisjoint(addend: MutableBigInteger, n: Int) {
        ...
        val result: IntArray
        if (value.size < resultLen) result = IntArray(resultLen) else {
            result = value
        ...
        value.copyInto(result, rstart + 1 - x, offset, offset + x)
        y -= x
        rstart -= x
    ...
    fun subtract(b: MutableBigInteger): Int {
        var b: MutableBigInteger = b
        var a: MutableBigInteger = this
        var result = value
        val sign: Int = a.compare(b)
        if (sign == 0) {
            reset()
            return 0
        }
        if (sign < 0) {
            val tmp: MutableBigInteger = a
            a = b
            b = tmp
        }
        val resultLen: Int = a.intLen
        ...
        while (y > 0) {
            x--
            y--
            diff = (a.value.get(x + a.offset).toLong() and BigInteger.LONG_MASK) -
                    (b.value.get(y + b.offset).toLong() and BigInteger.LONG_MASK) - (-(diff shr 32)).toInt()
            result[rstart--] = diff.toInt()
        }
        // Subtract remainder of longer number
        while (x > 0) {
            x--
            diff = (a.value.get(x + a.offset).toLong() and BigInteger.LONG_MASK) - (-(diff shr 32)).toInt()
            result[rstart--] = diff.toInt()
        }
        value = result
        intLen = resultLen
    ...
    private fun difference(b: MutableBigInteger): Int {
        ...
        while (y > 0) {
            x--
            y--
            diff = (a.value.get(a.offset + x).toLong() and BigInteger.LONG_MASK) -
                    (b.value.get(b.offset + y).toLong() and BigInteger.LONG_MASK) - (-(diff shr 32)).toInt()
            a.value[a.offset + x] = diff.toInt()
        }
        // Subtract remainder of longer number
        while (x > 0) {
            x--
            diff = (a.value.get(a.offset + x).toLong() and BigInteger.LONG_MASK) - (-(diff shr 32)).toInt()
            a.value[a.offset + x] = diff.toInt()
        }
        a.normalize()
        return sign
    ...
    fun multiply(y: MutableBigInteger, z: MutableBigInteger) {
        ...
        var k = yLen + xLen - 1
        while (j >= 0) {
            val product: Long = (y.value.get(j + y.offset).toLong() and BigInteger.LONG_MASK) *
        ...
        for (i in xLen - 2 downTo 0) {
            ...
            while (j >= 0) {
                val product: Long = (y.value.get(j + y.offset).toLong() and BigInteger.LONG_MASK) *
                        (value[i + offset].toLong() and BigInteger.LONG_MASK) +
                        (z.value.get(k).toLong() and BigInteger.LONG_MASK) + carry
                z.value[k] = product.toInt()
                carry = product ushr 32
                j--
                k--
    ...
    fun mul(y: Int, z: MutableBigInteger) {
        ...
        val zval = if (z.value.size < intLen + 1) IntArray(intLen + 1) else z.value
        var carry: Long = 0
    ...
    fun divideOneWord(divisor: Int, quotient: MutableBigInteger): Int {
        ...
        while (--xlen > 0) {
            val dividendEstimate = remLong shl 32 or
                    (value[offset + intLen - xlen].toLong() and BigInteger.LONG_MASK)
            var q: Int
    ...
    fun divide(
        ...
        return if (b.intLen < BigInteger.BURNIKEL_ZIEGLER_THRESHOLD ||
    ...
    fun divideKnuth(
        ...
        if (intLen >= KNUTH_POW2_THRESH_LEN) {
            val trailingZeroBits: Int = min(lowestSetBit, b.lowestSetBit)
            if (trailingZeroBits >= KNUTH_POW2_THRESH_ZEROS * 32) {
                val a = MutableBigInteger(this)
                b = MutableBigInteger(b)
                a.rightShift(trailingZeroBits)
                b.rightShift(trailingZeroBits)
                val r: MutableBigInteger = a.divideKnuth(b, quotient)!!
    ...
    fun divideAndRemainderBurnikelZiegler(
        ...
        return if (r < s) {
            ...
            val m =
                1 shl 32 - (s / BigInteger.BURNIKEL_ZIEGLER_THRESHOLD).countLeadingZeroBits()
            val j = (s + m - 1) / m // step 2a: j = ceil(s/m)
            val n = j * m // step 2b: block length in 32-bit units
            val n32 = 32L * n // block length in bits
            val sigma =
                max(0, n32 - b.bitLength()).toInt() // step 3: sigma = max{T | (2^T)*B < beta^n}
            ...
            val a1: MutableBigInteger = aShifted.getBlock(t - 1, t, n) // the most significant block of a

            // step 7: z[t-2] = [a[t-1], a[t-2]]
            var z: MutableBigInteger = aShifted.getBlock(t - 2, t, n) // the second to most significant block
    ...
    private fun divide2n1n(
        ...
        val aUpper: MutableBigInteger = MutableBigInteger(this)
        aUpper.safeRightShift(32 * (n / 2)) // aUpper = [a1,a2,a3]
        keepLower(n / 2) // this = a4

        // step 3: q1=aUpper/b, r1=aUpper%b
        val q1 = MutableBigInteger()
        val r1: MutableBigInteger = aUpper.divide3n2n(b, q1)

        // step 4: quotient=[r1,this]/b, r2=[r1,this]%b
        addDisjoint(r1, n / 2) // this = [r1,this]
        val r2: MutableBigInteger = divide3n2n(b, quotient)
    ...
    private fun divide3n2n(
        ...
        val n: Int = b.intLen / 2 // half the length of b in ints

        // step 1: view this as [a1,a2,a3] where each ai is n ints or less; let a12=[a1,a2]
        val a12: MutableBigInteger = MutableBigInteger(this)
        a12.safeRightShift(32 * n)

        // step 2: view b as [b1,b2] where each bi is n ints or less
        val b1: MutableBigInteger = MutableBigInteger(b)
        b1.safeRightShift(n * 32)
        val b2: BigInteger = b.getLower(n)
    ...
    private fun getBlock(index: Int, numBlocks: Int, blockLength: Int): MutableBigInteger {
        ...
        val newVal: IntArray =
            value.copyOfRange(offset + intLen - blockEnd, offset + intLen - blockStart)
    ...
    fun divide(v: Long, quotient: MutableBigInteger): Long {
        ...
        return if (d == 0) (divideOneWord(v.toInt(), quotient).toLong() and BigInteger.LONG_MASK) else {
    ...
    private fun divideMagnitude(
        ...
        val shift: Int = div.value.get(div.offset).countLeadingZeroBits()
        ...
        for (j in 0 until limit - 1) {
            ...
            if (!skipCorrection) { // Correct qhat
                val nl: Long = rem.value.get(j + 2 + rem.offset).toLong() and BigInteger.LONG_MASK
                var rs = (qrem.toLong() and BigInteger.LONG_MASK shl 32 or nl.toInt().toLong())
                var estProduct =
                    ((dl.toLong() and BigInteger.LONG_MASK) * (qhat.toLong() and BigInteger.LONG_MASK))
                if (unsignedLongCompare(estProduct, rs)) {
                    ...
                    if (qrem.toLong() and BigInteger.LONG_MASK >= dhLong) {
                        ...
                        rs = (qrem.toLong() and BigInteger.LONG_MASK shl 32 or nl.toInt().toLong())
        ...
        var qrem = 0
        var skipCorrection = false
        val nh: Int = rem.value[limit - 1 + rem.offset]
        val nh2 = nh + -0x80000000
        val nm: Int = rem.value[limit + rem.offset]
        if (nh == dh) {
            qhat = 0.inv()
            qrem = nh + nm
            skipCorrection = qrem + -0x80000000 < nh2
        ...
        if (qhat != 0) {
            if (!skipCorrection) { // Correct qhat
                val nl: Long = rem.value.get(limit + 1 + rem.offset).toLong() and BigInteger.LONG_MASK
                var rs = (qrem.toLong() and BigInteger.LONG_MASK shl 32 or nl.toInt().toLong())
                var estProduct =
                    ((dl.toLong() and BigInteger.LONG_MASK) * (qhat.toLong() and BigInteger.LONG_MASK))
                if (unsignedLongCompare(estProduct, rs)) {
            ...
            borrow = if (needRemainder) mulsub(
                rem.value,
                divisor,
                qhat,
                dlen,
    ...
    private fun divideLongMagnitude(
        ...
        // D1 normalize the divisor
        val shift: Int = ldivisor.countTrailingZeroBits()
        if (shift > 0) {
        ...
        for (j in 0 until limit) {
            ...
            var qrem = 0
            var skipCorrection = false
            val nh: Int = rem.value[j + rem.offset]
            val nh2 = nh + -0x80000000
            val nm: Int = rem.value[j + 1 + rem.offset]
            if (nh == dh) {
                qhat = 0.inv()
                qrem = nh + nm
                skipCorrection = qrem + -0x80000000 < nh2
    ...
    private fun divaddLong(dh: Int, dl: Int, result: IntArray, offset: Int): Int {
        var carry: Long = 0
        var sum =
            ((dl.toLong() and BigInteger.LONG_MASK) + (result[1 + offset].toLong() and BigInteger.LONG_MASK))
    ...
    private fun mulsubLong(q: IntArray, dh: Int, dl: Int, x: Int, offset: Int): Int {
        ...
        q[offset--] = difference.toInt()
        var carry: Long = (product ushr 32) + if (difference and BigInteger.LONG_MASK > product.toInt().inv().toLong() and BigInteger.LONG_MASK) 1 else 0
    ...
    fun hybridGCD(b: MutableBigInteger): MutableBigInteger {
        ...
        while (b.intLen != 0) {
            if (abs(a.intLen - b.intLen) < 2) return a.binaryGCD(b)
    ...
    private fun binaryGCD(v: MutableBigInteger): MutableBigInteger {
        ...
        var v: MutableBigInteger = v
        var u: MutableBigInteger = this
        val r = MutableBigInteger()

        // step B1
        val s1: Int = u.lowestSetBit
        val s2: Int = v.lowestSetBit
        ...
        // step B2
        val uOdd = k == s1
        var t: MutableBigInteger = if (uOdd) v else u
        ...
        while (t.lowestSetBit.also { lb = it } >= 0) {
            ...
            if (u.intLen < 2 && v.intLen < 2) {
                var x: Int = u.value.get(u.offset)
                val y: Int = v.value.get(v.offset)
                x = binaryGcd(x, y)
                r.value[0] = x
                r.intLen = 1
                r.offset = 0
    ...
    fun mutableModInverse(p: MutableBigInteger): MutableBigInteger {
        ...
        if (isEven) throw ArithmeticException("BigInteger not invertible.")

        // Get even part of modulus expressed as a power of 2
        val powersOf2: Int = p.lowestSetBit

        // Construct odd part of modulus
        val oddMod: MutableBigInteger = MutableBigInteger(p)
        oddMod.rightShift(powersOf2)
        if (oddMod.isOne) return modInverseMP2(powersOf2)

        // Calculate 1/a mod oddMod
        val oddPart: MutableBigInteger = modInverse(oddMod)

        // Calculate 1/a mod evenMod
        val evenPart: MutableBigInteger = modInverseMP2(powersOf2)

        // Combine the results using Chinese Remainder Theorem
        val y1: MutableBigInteger = modInverseBP2(oddMod, powersOf2)
        val y2: MutableBigInteger = oddMod.modInverseMP2(powersOf2)
    ...
    fun modInverseMP2(k: Int): MutableBigInteger {
        ...
        var t: Int = inverseMod32(value[offset + intLen - 1])
        if (k < 33) {
        ...
        tLong *= 2 - pLong * tLong // 1 more Newton iter step
    ...
    private fun modInverse(mod: MutableBigInteger): MutableBigInteger {
        ...
        if (f.isEven) {
            val trailingZeros: Int = f.lowestSetBit
        ...
        while (!f.isOne) {
            ...
            // If f == g (mod 4)
            if (f.value.get(f.offset + f.intLen - 1) xor
                g.value.get(g.offset + g.intLen - 1) and 3 == 0
            ) {
                f.subtract(g)
                c.signedSubtract(d)
            } else { // If f != g (mod 4)
    ...
    fun euclidModInverse(k: Int): MutableBigInteger {
        ...
        var r: MutableBigInteger = b.divide(a, q)!!
        ...
        while (!b.isOne) {
            ...
            swapper = r
            a = swapper
            if (q.intLen == 1) t1.mul(q.value.get(q.offset), temp) else q.multiply(t1, temp)
            swapper = q
            q = temp
            temp = swapper
            t0.add(q)
            ...
            swapper = b
            b = r
            if (q.intLen == 1) t0.mul(q.value.get(q.offset), temp) else q.multiply(t0, temp)
            swapper = q
            q = temp
            temp = swapper
            t1.add(q)
    ...
    companion object {
         ...
         */
        const val KNUTH_POW2_THRESH_LEN = 6
         ...
         */
        const val KNUTH_POW2_THRESH_ZEROS = 3
        ...
        fun binaryGcd(a: Int, b: Int): Int {
            ...
            val aZeros: Int = a.countTrailingZeroBits()
            val bZeros: Int = b.countTrailingZeroBits()
            a = a ushr aZeros
            b = b ushr bZeros
        ...
        fun fixup(
            ...
            val temp: MutableBigInteger = MutableBigInteger()
            // Set r to the multiplicative inverse of p mod 2^32
            val r: Int = -inverseMod32(p.value.get(p.offset + p.intLen - 1))
            var i = 0
            val numWords = k shr 5
            while (i < numWords) {

                // V = R * c (mod 2^j)
                val v: Int = r * c.value.get(c.offset + c.intLen - 1)
                // c = c + (v * p)
                p.mul(v, temp)
                c.add(temp)
                // c = c / 2^j
            ...
            if (numBits != 0) {
                // V = R * c (mod 2^j)
                var v: Int = r * c.value.get(c.offset + c.intLen - 1)
