...
class MathContext {
    ...
    constructor(`val`: String?) {
        ...
        if (`val` == null) throw NullPointerException("null String")
        try { // any error here is a string format problem
            ...
            val off: Int // where value starts
            setPrecision = `val`.substring(10, fence).toInt()
            if (!`val`.startsWith("roundingMode=", fence + 1)) throw RuntimeException()
            off = fence + 1 + 13
    ...
    companion object {
        ...
        private val DEFAULT_ROUNDINGMODE: RoundingMode = RoundingMode.HALF_UP
